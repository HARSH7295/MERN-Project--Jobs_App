...
const Job = (props) => {
    ...
    const {company,jobtype,location,position,status,createdAt,_id} = props.para
    ...
    {
        let response = await fetch(`http://127.0.0.1:8000/api/job/deleteJob/${id}`,{
            method:'DELETE',
            headers:{
  ...
  return (
    <div className='result-tab'>
        ...
        <div className='result-tab-bottom-buttons'>
            <button className='result-tab-edit-btn' onClick={()=>navigate(`/dashboard/editJob/${_id}`)}>Edit</button>
            <button className='result-tab-del-btn' onClick={()=>deleteJob(_id)}>Delete</button>
        </div>
    </div>
  )
}
